...
pub async fn send_email(
    ...
    if let Some(msg_id) = reply_to_message_id {
        args.push("--reply-to-message-id".to_string());
        args.push(msg_id);
    }
    ...
    let output = Command::new(&gog_path)
        .args(&args)
        .output()
        .map_err(|e| format!("Failed to run gog gmail send: {}", e))?;
...
/// Create an email draft via gog CLI.
...
/// Search Gmail messages via gog CLI.
pub async fn search_email(
    ...
    let output = Command::new(&gog_path)
        .args(&args)
        .output()
        .map_err(|e| format!("Failed to search email: {}", e))?;

    if output.status.success() {
        Ok(String::from_utf8_lossy(&output.stdout).trim().to_string())
    } else {
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(format!("Failed to search email: {}", stderr))
    }
}

/// Create a calendar event via gog CLI.
pub async fn create_calendar_event(
    ...
    if let Some(att) = attendees {
        for email in att.split(',') {
            args.push("--attendee".to_string());
    ...
    if output.status.success() {
        ...
        let stderr = String::from_utf8_lossy(&output.stderr);
        Err(format!("Failed to create event: {}", stderr))
    }
}
